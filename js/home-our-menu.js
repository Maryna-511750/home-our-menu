...
/*for (let page in  menuPages){
    renderMenuPosition(page, "desserts");
};
activatePage(previousMenuItem, "desserts");*/

renderMenuPosition(menuPages[0]);

let menuNavSectionsButton = document.querySelectorAll(".menu-nav-sections-button");
//let previousMenuItem = null;
//const menuPages = document.querySelectirAll(".menu-pages");
menuNavSectionsButton.forEach((button) => {
    button.addEventListener("click", (_) => {
        /*document.querySelectirAll(".menu-pages").forEach((menuPage) => {
            menuPage.style.display = 'none';
        });
        const menuPage = document.querySelector(`#${button.dataset.name}`);
        menuPage.style.display = 'block';*/
        renderMenuPosition(menuPages[`${button.dataset.id}`]);
        /*const menuPage = document.querySelector(`#${button.dataset.name}`);
        activatePage(previousMenuItem, menuPage);*/
    });
});

/*function activatePage (previousMenuItem, menuPage) {
    if (previousMenuItem !== null) {
        previousMenuItem.classList.remove("active-menu-pages");
    }
    menuPage.classList.add("active-menu-pages");
    previousMenuItem = menuPage;
};*/